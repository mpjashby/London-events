...
# EXTRACT SEPARATE CARNIVAL-DAY RESULTS -------------------------------

# Store the day-specific model indicators and labels so the same extraction
# steps can be applied to Carnival Sunday and Carnival Monday.
nhc_day_model_types <- tibble(
  carnival_day = c("sunday", "monday"),
  carnival_indicator = c("nhc_sunday", "nhc_monday")
)

# Extract and save results for each fitted Sunday/Monday crime-type model.
nhc_crime_groups |>
  walk(function(crime_group_name) {
    # Load the fitted day-specific model for the current crime group.
    nhc_day_model <- read_rds(
      here("derived_data", str_glue("nhc_model_day_{crime_group_name}.rds"))
    )

    # Extract day-specific model coefficients with 95% confidence intervals.
    day_coef_results <- tidy(
      nhc_day_model,
      conf.int = TRUE,
      conf.level = 0.95
    ) |>
    ...
    # Save the day-specific coefficient table for the current crime group.
    write_rds(
      day_coef_results,
      here("derived_data", str_glue("nhc_coef_day_{crime_group_name}.rds")),
      compress = "gz"
    )

    # Store model fit statistics in a compact object for reporting in the paper.
    nhc_day_fit_stats <- fitstat(
      nhc_day_model,
      c("ll", "aic", "bic", "rmse", "sq.cor", "pr2", "apr2")
    )

    # Test whether all estimated Sunday and Monday distance-band coefficients
    # are jointly equal to zero.
    nhc_day_band_test <- wald(nhc_day_model, keep = "nhc_sunday|nhc_monday")

    # Test whether all estimated Sunday distance-band coefficients are jointly
    # equal to zero.
    nhc_sunday_band_test <- wald(nhc_day_model, keep = "nhc_sunday")

    # Test whether all estimated Monday distance-band coefficients are jointly
    # equal to zero.
    nhc_monday_band_test <- wald(nhc_day_model, keep = "nhc_monday")

    # Store the fixed-effect levels dropped by the model because they had only
    # zero outcomes.
    nhc_day_dropped_fixed_effects <- tibble(
      fixed_effect = names(nhc_day_model$fixef_removed),
      dropped_level = nhc_day_model$fixef_removed
    ) |>
      # Store all dropped fixed-effect levels as labels so different fixed-effect
      # types can be combined in one tidy table.
      mutate(dropped_level = map(dropped_level, as.character)) |>
    ...
    # Store a compact count of rows removed at each model-selection step.
    nhc_day_dropped_row_summary <- enframe(
      nhc_day_model$obs_selection,
      name = "drop_step",
      value = "row_indices"
    ) |>
      mutate(rows_dropped = lengths(row_indices)) |>
    ...
    # Store model-level statistics that are useful for manuscript tables and
    # appendix reporting.
    day_stats_results <- tibble(
      crime_group = crime_group_name,
      observations_original = nhc_day_model$nobs_origin,
      observations_dropped = nhc_day_model$nobs_origin - nhc_day_model$nobs,
      observations_remaining = nhc_day_model$nobs,
      dropped_row_summary = list(nhc_day_dropped_row_summary),
      dropped_fixed_effects = list(nhc_day_dropped_fixed_effects),
      dropped_terms = list(nhc_day_model$collin.var),
      hexagon_fixed_effects = nhc_day_model$fixef_sizes[["hex_id"]],
      date_fixed_effects = nhc_day_model$fixef_sizes[["crime_date"]],
      log_likelihood = unname(nhc_day_fit_stats$ll),
      aic = unname(nhc_day_fit_stats$aic),
      bic = unname(nhc_day_fit_stats$bic),
      rmse = unname(nhc_day_fit_stats$rmse),
      squared_correlation = unname(nhc_day_fit_stats$sq.cor),
      pseudo_r2 = unname(nhc_day_fit_stats$pr2),
      adjusted_pseudo_r2 = unname(nhc_day_fit_stats$apr2),
      joint_band_test_statistic = unname(nhc_day_band_test$stat),
      joint_band_test_p_value = unname(nhc_day_band_test$p),
      joint_band_test_df1 = unname(nhc_day_band_test$df1),
      joint_band_test_df2 = unname(nhc_day_band_test$df2),
      joint_band_test_vcov = nhc_day_band_test$vcov,
      sunday_band_test_statistic = unname(nhc_sunday_band_test$stat),
      sunday_band_test_p_value = unname(nhc_sunday_band_test$p),
      sunday_band_test_df1 = unname(nhc_sunday_band_test$df1),
      sunday_band_test_df2 = unname(nhc_sunday_band_test$df2),
      monday_band_test_statistic = unname(nhc_monday_band_test$stat),
      monday_band_test_p_value = unname(nhc_monday_band_test$p),
      monday_band_test_df1 = unname(nhc_monday_band_test$df1),
      monday_band_test_df2 = unname(nhc_monday_band_test$df2)
    )

    # Save the compact day-specific model statistics for the current crime group.
    write_rds(
      day_stats_results,
      here("derived_data", str_glue("nhc_stats_day_{crime_group_name}.rds")),
      compress = "gz"
    )

    # Store the model coefficients for use in the day-specific extra-crime
    # calculations.
    nhc_day_model_coefs <- coef(nhc_day_model)

    # Store the model variance-covariance matrix so the London-wide intervals
    ...
    day_extra_results <- map2(
      ...
      function(day_label, day_indicator) {
          ...
          carnival_day_annual_scale <- 1 / n_distinct(nhc_day_data$crime_date)

          # Create a counterfactual version of the day rows where the current
          # Carnival-day indicator is switched off.
          no_nhc_day_data <- nhc_day_data
          no_nhc_day_data[[day_indicator]] <- 0

          # Estimate additional crimes predicted on the current Carnival day by
          # distance band.
          distance_extra_results <- nhc_day_data |>
            mutate(
              predicted_nhc = predict(
                nhc_day_model,
                newdata = nhc_day_data,
            ...
            summarise(
              observed_nhc = sum(crime_count) * carnival_day_annual_scale,
              predicted_nhc = sum(predicted_nhc) * carnival_day_annual_scale,
              predicted_no_nhc = sum(predicted_no_nhc) * carnival_day_annual_scale,
              .by = c(dist_km, dist)
            ) |>
            # Link each distance band to the corresponding day-specific model
            # coefficient.
            mutate(
              model_term = if_else(
                dist == "12km",
                NA_character_,
                str_glue("dist::{dist}:{day_indicator}")
              ),
              nhc_effect = if_else(
                is.na(model_term),
                0,
                nhc_day_model_coefs[model_term]
              ),
              nhc_effect_se = if_else(
                is.na(model_term),
                0,
                sqrt(diag(nhc_day_model_vcov)[model_term])
              ),
              extra_crimes = predicted_no_nhc * (exp(nhc_effect) - 1),
              extra_crimes_se = predicted_no_nhc * exp(nhc_effect) * nhc_effect_se,
              extra_crimes_conf_low = extra_crimes -
          ...
          # Store the model terms that contribute uncertainty to the
          # London-wide day-specific total.
          total_model_terms <- distance_extra_results |>
            filter(!is.na(model_term)) |>
            pull(model_term)

          # Store the derivative of the London-wide day-specific extra-crime
          # estimate with respect to each distance-band coefficient.
          total_extra_crimes_gradient <- distance_extra_results |>
            filter(!is.na(model_term)) |>
            mutate(extra_crimes_gradient = predicted_no_nhc * exp(nhc_effect)) |>
            pull(extra_crimes_gradient)

          # Calculate the standard error for the London-wide total using the
          # full covariance matrix for the distance-band coefficients.
          total_extra_crimes_se <- sqrt(as.numeric(
            t(total_extra_crimes_gradient) %*%
              nhc_day_model_vcov[total_model_terms, total_model_terms, drop = FALSE] %*%
              total_extra_crimes_gradient
          ))

          # Add the crime-group label, Carnival-day label and London-wide total
          # row to the day-specific results.
          bind_rows(
            distance_extra_results |>
              mutate(
                crime_group = crime_group_name,
                carnival_day = day_label,
                dist = as.character(dist),
                .before = 1
              ),
            distance_extra_results |>
            ...
            # Order distance bands from the footprint out, with the total row
            # last.
            arrange(carnival_day, dist_km)
      }
    ) |>
      list_rbind()

    # Save the day-specific extra-crime estimates for the current crime group.
    write_rds(
      day_extra_results,
      here("derived_data", str_glue("nhc_extra_day_{crime_group_name}.rds")),
      compress = "gz"
    )
  })
