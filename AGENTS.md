...
* Always start each script by loading the required packages using `pacman::p_load()`. The last package loaded should always be tidyverse, which should be loaded in all cases.
* Each line or block of code should be commented to explain what it does.
* Functions should not be created where the loss of readability outweighs the robustness benefit.
* The project structure is static and this project will not be deployed in a production environment, so avoid adding code to check project structure, etc., since this increases complexity for no benefit.
* The data is static, so do not add code to check on data formats or structure.
* Do not create objects to store values that will only be used once.
* Never select columns by position, only by name.
* Do not include calls to `message()` or similar in the code.
* Do not suppress output or progress bars.
* Apply `janitor::clean_names()` immediately after loading a new dataset that was not created by another script in the same project. Do this before any other operations, so that other operations can rely on column names being in lower case. When loading multiple datasets using `map()` or similar, add `map(janitor::clean_names)` as the next item in the code pipeline, or the first item in the pipeline after `bind_rows()` as appropriate.
* Do not start code with `#!/usr/bin/env Rscript` since the code will be run in Positron
* If a package has been loaded at the start of a script, do not refer to functions from that package using `::` notation.
* Always follow the tidyverse style guide: https://style.tidverse.org/
